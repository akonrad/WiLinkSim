...
public class modelsMenu {
	...
	JLabel label = new JLabel("Choose your model preference for the wireless modelling");
	...
	proceed.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			traceDialogue tracer = new traceDialogue("modelsMenu");
			tracer.getTraceFile();
		    }
		}
	    });
	...
	back.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
		    frame.dispose();
		    if (previous.equals("errorOrDelayMenu")) {
			errorOrDelayMenu temp = new errorOrDelayMenu();
			temp.errorOrDelay();
		    }
		    else if (previous.equals("wirelessFeedback")) {
			wirelessFeedback temp = new wirelessFeedback();
			temp.showMenu();
		    }
		    else if (previous.equals("wirelessFECMenu")) {
			wirelessFECMenu temp = new wirelessFECMenu();
			temp.displayFEC();
		    }
		    else if (previous.equals("newNetwork")) {
			newNetwork temp = new newNetwork();
			temp.runNewNetworks();
		    }
		}
	...
	try {
	    ...
	    String data = "<WirelessErrorModel> " + model + " </WirelessErrorModel>\n";
	    optionWriter.write(data, 0, data.length());
	    optionWriter.flush();
	    optionWriter.close();
	} catch (Exception e) {
	    System.out.println("Error while writing to options file in modelsMenu.java");
	}
    }
