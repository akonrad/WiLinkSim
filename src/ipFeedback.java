...
/**
 * Creates the GUI to find out if feedback is required once IP error modelling is chosen.
...
public class ipFeedback {
	...
	final JFrame frame = new JFrame("Feeback selection for IP Modelling");
	...
	JLabel label = new JLabel("Choose whether you want feedback simulation for the IP level modelling");
	...
	try {
	    ...
	    String data = "<IP_Feedback> " + feedback + " </IP_Feedback>\n";
	    options.write(data, 0, data.length());
	    options.flush();
	    options.close();
	} catch (Exception e){
	    System.out.println("Exception while writing options in ipFeedback.java");
	}
	...
	if (!(feedbackChoice))
	    {
		ipModelsMenu temp = new ipModelsMenu("ipFeedback");
		temp.makeModelMenu();
	    }
	else 
	    {
